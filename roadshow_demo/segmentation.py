...
def SegImg(img, READY, STAGE, LIMIT=None, GAME=True, SUCCESS=False, FAIL=False):
    ...
    if READY and GAME == False:
        ...
        '''
        if not STAGE.determine_stage(img=None, flag=False, parts_config=parts_config, person_keypoints=person_keypoints):
            SUCCESS = False
            FAIL = True
        ...
        for i in range(LIMIT) :
            ls = person_keypoints[i][5]
            le = person_keypoints[i][7]
            lw = person_keypoints[i][9]

            rs = person_keypoints[i][6]
            re = person_keypoints[i][8]
            rw = person_keypoints[i][10]

            # lw [x, y, 1] form
            if lw[1] > ls[1] or rw[1] > rs[1] :
                SUCCESS = False
                FAIL = True
                print("keypoints fail")
                break
        ...
        num_label_s = []
        min_ang = np.array([20.0])
        #print("\nparts_angles\n", parts_angles)
        for idx, part in enumerate(parts_angles):
            # CHECK!
            if any([part[1], part[7]] < min_ang) or any([part[2], part[8]] < min_ang):
                SUCCESS = False
                FAIL = True
                print(idx)
                print(part)
